...
// Minimal libusb-1.0 ABI surface used by RPCS3's emulated USB subsystem.
// iOS does not expose arbitrary host USB passthrough to applications. The
// companion implementation returns LIBUSB_ERROR_NOT_SUPPORTED for physical
// device operations while preserving transfer structures and helper functions
// used by fully emulated PS3 USB devices.
...
#define LIBUSB_TRANSFER_SHORT_NOT_OK (1U << 0)
#define LIBUSB_TRANSFER_FREE_BUFFER (1U << 1)
#define LIBUSB_TRANSFER_FREE_TRANSFER (1U << 2)
#define LIBUSB_TRANSFER_ADD_ZERO_PACKET (1U << 3)
...
static inline void libusb_fill_control_transfer(struct libusb_transfer* transfer,
    libusb_device_handle* handle, unsigned char* buffer, libusb_transfer_cb_fn callback,
    void* user_data, unsigned int timeout)
{
    struct libusb_control_setup* setup = (struct libusb_control_setup*)buffer;
    transfer->dev_handle = handle;
    transfer->endpoint = setup ? (setup->bmRequestType & LIBUSB_ENDPOINT_DIR_MASK) : 0;
    transfer->type = LIBUSB_TRANSFER_TYPE_CONTROL;
    transfer->timeout = timeout;
    transfer->buffer = buffer;
    transfer->length = setup ? LIBUSB_CONTROL_SETUP_SIZE + libusb_cpu_to_le16(setup->wLength) : 0;
    transfer->user_data = user_data;
    transfer->callback = callback;
}

static inline void libusb_fill_bulk_transfer(struct libusb_transfer* transfer,
    libusb_device_handle* handle, unsigned char endpoint, unsigned char* buffer,
...
static inline void libusb_fill_interrupt_transfer(struct libusb_transfer* transfer,
    ...
    int length, libusb_transfer_cb_fn callback, void* user_data, unsigned int timeout)
{
    libusb_fill_bulk_transfer(transfer, handle, endpoint, buffer, length, callback, user_data, timeout);
    transfer->type = LIBUSB_TRANSFER_TYPE_INTERRUPT;
}

static inline void libusb_fill_iso_transfer(struct libusb_transfer* transfer,
    libusb_device_handle* handle, unsigned char endpoint, unsigned char* buffer,
    int length, int packets, libusb_transfer_cb_fn callback, void* user_data,
    unsigned int timeout)
{
    libusb_fill_bulk_transfer(transfer, handle, endpoint, buffer, length, callback, user_data, timeout);
...
static inline unsigned char* libusb_get_iso_packet_buffer_simple(struct libusb_transfer* transfer,
    unsigned int packet)
{
    unsigned int offset = 0;
    for (unsigned int index = 0; index < packet && index < (unsigned int)transfer->num_iso_packets; ++index)
    {
        offset += transfer->iso_packet_desc[index].length;
    }
    return transfer->buffer ? transfer->buffer + offset : NULL;
}
