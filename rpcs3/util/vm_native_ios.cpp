...
namespace utils
{
	...
	// iOS core builds. Non-JIT decommits continue through the original code.
	...
	void memory_decommit(void* pointer, usz size, bool can_be_jit)
	{
		...
		// MAP_FIXED. The inherited macOS path unmaps first and then only supplies
		// the address as a hint, which can move RPCS3's JIT arena. Keep the
